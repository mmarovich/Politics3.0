...
function createAccount() {
	$('.createAccount').submit(function(e) {
		...
		$.ajax({
			...
			error: function() {
				$('.signUp > span').html('User already taken');
...
function logIn() {
	$('.logIn').submit(function(e) {
		...
		$.ajax({
			...
			error: function() {
				$('.logInBox > span').html('Invalid Username/password');
...
function presidentData(data, i) {
	const rootData = data.officials[data.offices[i].officialIndices[0]];
	$('.president > .name > span').html(rootData.name);
	$('.president > .party > span').html(rootData.party);
	$('.president > .address > span').html(rootData.address[0].line1 + '<br>' +
		(rootData.address[0].line2 ? 
			rootData.address[0].line2 + '<br>': '') +
		rootData.address[0].city + ', ' +
		rootData.address[0].state + ' ' +
		rootData.address[0].zip);
	$('.president > .phone > span').html(rootData.phones[0]);
	$('.president > .website > a').html(rootData.urls?
		rootData.urls[0] : '').attr("href", rootData.urls ?
		rootData.urls[0] : '');
}

function vpData(data, i) {
	const rootData = data.officials[data.offices[i].officialIndices[0]];
	$('.vicePresident > .name > span').html(rootData.name);
	$('.vicePresident > .party > span').html(rootData.party);
	$('.vicePresident > .address > span').html(rootData.address[0].line1 + '<br>' +
		(rootData.address[0].line2 ? 
			rootData.address[0].line2 + '<br>': '') +
		rootData.address[0].city + ', ' +
		rootData.address[0].state + ' ' +
		rootData.address[0].zip);
	$('.vicePresident > .phone > span').html(rootData.phones[0]);
	$('.vicePresident > .website > a').html(rootData.urls ?
		rootData.urls[0] : '').attr("href", rootData.urls ?
		rootData.urls[0] : '');
}
...
function senateData(data, i) {
	const rootData = data.officials[data.offices[i].officialIndices[0]];
	const rootData2 = data.officials[data.offices[i].officialIndices[1]];
	$('.senate1 > .name > span').html(rootData.name);
	$('.senate1 > .party > span').html(rootData.party);
	$('.senate1 > .address > span').html(rootData.address[0].line1 + '<br>' +
		(rootData.address[0].line2 ? 
			rootData.address[0].line2 + '<br>': '') +
		rootData.address[0].city + ', ' +
		rootData.address[0].state + ' ' +
		rootData.address[0].zip);
	$('.senate1 > .phone > span').html(rootData.phones[0]);
	$('.senate1 > .website > a').html(rootData.urls ?
		rootData.urls[0] : '').attr("href", rootData.urls ?
		rootData.urls[0] : '');
	$('.senate2 > .name > span').html(rootData2.name);
	$('.senate2 > .party > span').html(rootData2.party);
	$('.senate2 > .address > span').html(rootData2.address[0].line1 + '<br>' +
		(rootData2.address[0].line2 ? 
			rootData2.address[0].line2 + '<br>': '') +
		rootData2.address[0].city + ', ' +
		rootData2.address[0].state + ' ' +
		rootData2.address[0].zip);
	$('.senate2 > .phone > span').html(rootData2.phones[0]);
	$('.senate2 > .website > a').html(rootData2.urls ?
		rootData2.urls[0] : '').attr("href", rootData2.urls ?
		rootData2.urls[0] : '');
}

function governorData(data, i) {
	const rootData = data.officials[data.offices[i].officialIndices[0]];
	$('.governor > .name > span').html(rootData.name);
	$('.governor > .party > span').html(rootData.party);
	$('.governor > .address > span').html(rootData.address[0].line1 + '<br>' +
		(rootData.address[0].line2 ? 
			rootData.address[0].line2 + '<br>': '') +
		rootData.address[0].city + ', ' +
		rootData.address[0].state + ' ' +
		rootData.address[0].zip);
	$('.governor > .phone > span').html(rootData.phones[0]);
	$('.governor > .website > a').html(rootData.urls ?
		rootData.urls[0] : '').attr("href", rootData.urls ?
		rootData.urls[0] : '');
}
...
function mayorData(data, i) {
	const rootData = data.officials[data.offices[i].officialIndices[0]];
	$('.mayor > .name > span').html(rootData.name);
	$('.mayor > .party > span').html(rootData.party);
	$('.mayor > .address > span').html(rootData.address[0].line1 + '<br>' +
		(rootData.address[0].line2 ? 
			rootData.address[0].line2 + '<br>': '') +
		rootData.address[0].city + ', ' +
		rootData.address[0].state + ' ' +
		rootData.address[0].zip);
	$('.mayor > .phone > span').html(rootData.phones[0]);
	$('.mayor > .website > a').html(rootData.urls ?
		rootData.urls[0] : '').attr("href", rootData.urls ?
		rootData.urls[0] : '');
}
...
function cityClerkData(data, i) {
	const rootData = data.officials[data.offices[i].officialIndices[0]];
	$('.cityClerk > .name > span').html(rootData.name);
	$('.cityClerk > .party > span').html(rootData.party);
	$('.cityClerk > .address > span').html(rootData.address[0].line1 + '<br>' +
		(rootData.address[0].line2 ? 
			rootData.address[0].line2 + '<br>': '') +
		rootData.address[0].city + ', ' +
		rootData.address[0].state + ' ' +
		rootData.address[0].zip);
	$('.cityClerk > .phone > span').html(rootData.phones[0]);
	$('.cityClerk > .website > a').html(rootData.urls ?
		rootData.urls[0] : '').attr("href", rootData.urls ?
		rootData.urls[0] : '');
}
